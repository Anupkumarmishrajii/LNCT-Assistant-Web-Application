...
using WebLNCTAssistant.Areas.Students.Models;

namespace WebLNCTAssistant.Areas.Students.Controllers
{
    [Area("Students")]
    [Route("Students")]
    public class StudentsController : Controller
    {
        private const string CaptchaSessionKey = "StudentCaptchaValue";
        ...
        public async Task<IActionResult> StudentLogin()
        {
            try
            {
                ...
                // Generate the captcha image URL
                var captchaImageUrl = Url.Action("CaptchaImage", "Students", new { code = captchaCode });
                ...
                StudentsModel objStudentsModel = new StudentsModel();
                return View(objStudentsModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route("StudentLogin")]
        public async Task<IActionResult> StudentLogin(StudentsModel objStudentsModel)
        {
            try
            {
                if (!ModelState.IsValid)
                {
                    objStudentsModel.IsShowValidation = false;
                    objStudentsModel.ErrorMessage = "validate the model.";
                    return View(objStudentsModel);
                }
                var OriginalCaptchaCode = HttpContext.Session.GetString(CaptchaSessionKey);
                HttpContext.Session.Remove(CaptchaSessionKey);

                string? InputEmail = objStudentsModel.Email;
                StudentsDTO objStudentsDTO1 = await BALFactory.Instance.StudentsBAL.GetPasswordByEmail(InputEmail);
                if (objStudentsDTO1 != null && objStudentsDTO1.Password == objStudentsModel.Password)
                {
                    if (objStudentsModel.Captcha == OriginalCaptchaCode)
                    {
                        return RedirectToAction("StudentLoginSuccessMessage");
                    }
                    else
                    {
        ...
        [HttpGet]
        [Route("StudentLoginSuccessMessage")]
        public async Task<IActionResult> StudentLoginSuccessMessage()
        {
            return View("_StudentLoginSuccessMessage");
        }
